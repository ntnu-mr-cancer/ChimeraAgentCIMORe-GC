...
def load_prediction(
    ...
    case_dir = PREDICTIONS_DIR / task_id / case_id

    decision_name, reasoning_name = PREDICTION_FILES[task_id]

    decision_path = case_dir / decision_name
    reasoning_path = case_dir / reasoning_name

    missing = [
        p.name
        for p in (decision_path, reasoning_path)
        if not p.exists()
    ]

    if missing:
        return None, f"missing candidate file(s): {missing}"
    ...
    if task_id == "task1":
        ...
        pred = {
            "months_to_recurrence": decision.get(
...
#     g = _norm_conf(gt.get("confidence"))
#     p = _norm_conf(pred.get("confidence"))
#     print(f"DEBUG confidence_score: gt_raw={gt.get('confidence')!r} → g={g!r} | "
#           f"pred_raw={pred.get('confidence')!r} → p={p!r}")
#     if g is None or p is None:
#         return None
#     distance = abs(CONF_MAP[g] - CONF_MAP[p])
#     max_dist = max(CONF_MAP.values()) - min(CONF_MAP.values())  # = 2
#     score = 1.0 - (distance / max_dist)
#     print(f"DEBUG confidence_score: CONF_MAP[{g}]={CONF_MAP[g]}, CONF_MAP[{p}]={CONF_MAP[p]}, "
#           f"distance={distance}, score={score}")
#     return score
...
def run() -> None:
    """
    Evaluate a single task using direct case-directory matching.

    TASKS environment variable selects the task:
        TASKS=1 -> task1
        TASKS=2 -> task2
        TASKS=3 -> task3
    ...
    Candidate and ground-truth records are matched directly by case
    directory name. No predictions.json, job PK, socket PK, or archive
    mapping is used.
    """
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    task_num = os.getenv("TASK", "1").strip()
    task_id = f"task{task_num}"
    # task_id = f"task{3}"

    ground_truth_task_dir = GROUND_TRUTH_DIR / task_id / "agent_input" 
    ...
    if not predictions_task_dir.exists():
        raise RuntimeError(
            f"Missing predictions directory: {predictions_task_dir}"
        )
    ...
    if not ground_truth_by_case:
        raise RuntimeError(
            f"No ground-truth cases found in "
            f"{ground_truth_task_dir}"
        )

    print(
        f"Loaded {len(ground_truth_by_case)} {task_id} "
        f"ground-truth cases from {ground_truth_task_dir}"
    )

    print(
        f"Reading candidate outputs from "
    ...
    for case_id in tqdm(sorted(ground_truth_by_case)):
        ...
        pred, load_status = load_prediction(
            task_id=task_id,
            case_id=case_id,
            ground_truth=gt,
        )
    ...
    aggregate_task = compute_aggregate_metrics(rows)

    ranking_score = aggregate_task.get("ranking_score")

    if ranking_score is None:
        raise RuntimeError(
            f"{task_id} ranking score is undefined"
        )
    ...
    print(
        f"{task_id}: "
        f"cases={aggregate_task.get('n_cases', 0)} "
        f"evaluated={aggregate_task.get('n_evaluated', 0)} "
